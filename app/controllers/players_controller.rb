class PlayersController < ApplicationController
  ...
  def show
    ...
    if player_data.summary.success == false
      ...
      @player = User.find_by(uid: player_profile['steamid'].to_i - 76561197960265728)
      if !@player
        @player = User.create(uid: (player_profile['steamid'].to_i - 76561197960265728).to_s, nickname: player_profile['personaname'], avatar_url: player_profile['avatarmedium'], profile_url: player_profile['profileurl'])
      elsif (@player.nickname != player_profile['personaname'] || @player.avatar != player_profile['avatarmedium'])
